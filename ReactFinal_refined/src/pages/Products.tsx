...
export default function Products() {
  ...
  const [queryInput, setQueryInput] = useState<string>(getStored('queryInput', ''));
  const [query, setQuery] = useState<string>(getStored('query', ''));
  const [selectedColor, setSelectedColor] = useState<string[]>(getStored('selectedColor', ['all']));
  const [selectedStorage, setSelectedStorage] = useState<string[]>(getStored('selectedStorage', ['all']));
  const [selectedRam, setSelectedRam] = useState<string[]>(getStored('selectedRam', ['all']));
  const [selectedYear, setSelectedYear] = useState<string[]>(getStored('selectedYear', ['all']));
  const [selectedOS, setSelectedOS] = useState<string[]>(getStored('selectedOS', ['all']));
  const [selectedDiscount, setSelectedDiscount] = useState<string[]>(getStored('selectedDiscount', ['all']));
  const [selectedCategory, setSelectedCategory] = useState<string[]>(getStored('selectedCategory', ['all']));
  const [selectedDesigner, setSelectedDesigner] = useState<string[]>(getStored('selectedDesigner', ['all']));
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem('selectedDesigner', JSON.stringify(selectedDesigner));
  }, [
    queryInput,
    query,
    selectedColor,
  ...
  const searchSuggestions = queryInput.length > 0 
    ? allProducts
        .filter(item => 
  ...
  const storages = Array.from(
    new Set(allProducts.map((item) => (item as any).storage).filter(Boolean))
  );
  ...
  const operatingSystems = Array.from(
    new Set(
      allProducts.map((item) => (item as any).operating_system).filter(Boolean)
    )
  );
  ...
  const filterConfig = [
    { label: t('category'), options: categories, state: selectedCategory, setter: setSelectedCategory },
    { label: t('designer'), options: designers, state: selectedDesigner, setter: setSelectedDesigner },
    { label: t('color'), options: colors, state: selectedColor, setter: setSelectedColor },
    { label: t('storage'), options: storages.map((s) => s.toString()), state: selectedStorage, setter: setSelectedStorage },
    { label: t('ram'), options: rams.map((r) => r.toString()), state: selectedRam, setter: setSelectedRam },
    { label: t('year'), options: years.map((y) => y.toString()), state: selectedYear, setter: setSelectedYear },
    { label: t('os'), options: operatingSystems, state: selectedOS, setter: setSelectedOS },
    { label: t('discount'), options: [...discounts.map((d) => d.toString()), 'none'], state: selectedDiscount, setter: setSelectedDiscount },
  ] as const;
  ...
  const quickFilters = [
    { label: t('phones'), category: 'phones' },
    { label: t('earbuds'), category: 'earbuds' },
    { label: t('watches'), category: 'watches' },
    { label: t('cases'), category: 'cases' },
  ];
  ...
  return (
    <div className="products-page">
      ...
      <div className="quick-filters">
        <button
          className={`quick-filter-btn ${selectedCategory.includes('all') ? 'active' : ''}`}
        ...
        {quickFilters.map((filter) => (
          <button
            key={filter.category}
            className={`quick-filter-btn ${selectedCategory.includes(filter.category) ? 'active' : ''}`}
